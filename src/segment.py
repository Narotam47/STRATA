...
def write_phase2_report(
    sweep: pd.DataFrame,
    df: pd.DataFrame,
    single_trip_df: pd.DataFrame,
    cluster_id_to_name: dict,
    final_cols: list[str],
) -> None:
    """Write outputs/reports/phase2_segmentation.md with the k-selection
    rationale, cluster profiles, stability result, and central finding."""

    seg_order = ["Champions", "Loyal Actives", "Declining", "Truly Lapsed"]

    # ── Sweep table ───────────────────────────────────────────────────────────
    sweep_lines = ["| k | Silhouette | Inertia | Calinski-Harabasz |",
                   "|---|---:|---:|---:|"]
    for _, row in sweep.iterrows():
        marker = " ← metric peak" if int(row["k"]) == int(sweep.loc[sweep["silhouette"].idxmax(), "k"]) else ""
        sweep_lines.append(
            f"| {int(row['k'])} | {row['silhouette']:.4f}{marker} "
            f"| {row['inertia']:,.0f} | {row['ch']:.1f} |"
        )

    # ── Cluster profiles (raw untransformed medians) ──────────────────────────
    profile_rows = []
    for name in seg_order:
        sub = df[df["cluster_name"] == name]
        profile_rows.append(
            f"| {name} | {len(sub):,} "
            f"| {sub['frequency'].median():.0f} "
            f"| ${sub['monetary'].median():,.0f} "
            f"| {sub['recency_days'].median():.0f}d "
            f"| {sub['tenure_days'].median():.0f}d "
            f"| {sub['private_label_spend_share'].median():.1%} |"
        )

    # ── Report body ───────────────────────────────────────────────────────────
    sil_at_k4 = float(sweep[sweep["k"] == 4]["silhouette"].values[0])
    sil_peak_k = int(sweep.loc[sweep["silhouette"].idxmax(), "k"])
    sil_peak   = float(sweep["silhouette"].max())
...
Silhouette and Calinski-Harabasz both peak at **k = {sil_peak_k}**
(silhouette = {sil_peak:.4f}). At k = 4 the silhouette is {sil_at_k4:.4f}
— a deliberate and documented trade-off explained in Section 2.

---

## 2. Why k = 4, Not k = {sil_peak_k}

The metric optimum at k = {sil_peak_k} produces a trivially interpretable
...
The automated decision rule (silhouette ≥ 0.90 × peak) recommended k = {int(sweep.loc[sweep['silhouette'] >= 0.90 * sil_peak, 'k'].values[0])} on
...
**{len(df):,} households in k-means** ({len(single_trip_df)} Single-Trip households
...
**Features used in fit ({len(final_cols)}):**
`{", ".join(final_cols)}`
...
**Declining** (n ≈ {len(df[df['cluster_name']=='Declining']):,}): median tenure
{df[df['cluster_name']=='Declining']['tenure_days'].median():.0f} days,
median recency {df[df['cluster_name']=='Declining']['recency_days'].median():.0f} days.
...
**Truly Lapsed** (n ≈ {len(df[df['cluster_name']=='Truly Lapsed']):,}): median tenure
{df[df['cluster_name']=='Truly Lapsed']['tenure_days'].median():.0f} days,
median recency {df[df['cluster_name']=='Truly Lapsed']['recency_days'].median():.0f} days.
...
"""
    ...
    out_path = REPORTS_DIR / "phase2_segmentation.md"
    out_path.write_text(report, encoding="utf-8")
    print(f"\n  Phase 2 report written → {out_path.relative_to(PROJECT_ROOT)}")
...
def main():
    ...
    # Sorted by household_id so re-runs produce bit-identical files (no spurious git diffs).
    assign = df[["household_id", "cluster", "cluster_name",
                  "r_score", "f_score", "m_score",
                  "rfm_code", "has_demographics"]].copy()
    assign = assign.sort_values("household_id").reset_index(drop=True)
    ...
    write_phase2_report(sweep, df, single_trip_df, cluster_id_to_name, final_cols)
